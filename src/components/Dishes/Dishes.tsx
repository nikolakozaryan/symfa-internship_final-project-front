...
export const Dishes: FC<MyProps> = ({ pageType, bannerVisible }) => {
    ...
    const { dishes, page, pages, search } = useAppSelector(state => (isHome ? state.menu : state.fav));
    ...
    const firstTwoFavs = (
        <div className={styles.container}>
            <div className={styles.dishes}>
                {dishes.length ? (
                    dishes.slice(0, 2).map(dish => <DishCard key={dish.id} {...dish} />)
                ) : (
                    <NothingFound />
                )}
            </div>
    ...
    const allDishes = (
        <div className={styles.container} onScroll={handleScroll}>
            <div className={styles.dishes}>
                {dishes.length ? dishes.map(dish => <DishCard key={dish.id} {...dish} />) : <NothingFound />}
            </div>
        </div>
    );
