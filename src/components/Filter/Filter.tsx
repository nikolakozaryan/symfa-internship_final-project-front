...
export const Filter: FC<MyProps> = ({ pageType }) => {
    ...
    const TypeList = DISH_TYPES.map(type => (
        <FilterItem key={type} pageType={pageType} active={type === dishType} type={type} />
    ));

    const TasteList = DISH_TASTE.map(taste => (
        <FilterItem key={taste} pageType={pageType} active={taste === dishTaste} taste={taste} />
    ));

    return (
        <div className={styles.filter__container} onWheel={handleScroll}>
            <ul className={styles.filter}>{pageType === MenuRoutes.Home ? TypeList : TasteList}</ul>
        </div>
    );
};
